"""
UI/CreateMatch.py
Phòng chờ sau khi tạo/join phòng — kết nối Railway socket thật.
"""
...
class CreateMatch:
    ...
    def __init__(self, screen_w, screen_h, pin: str, host: str,
        ...
        self._msg_timer = 0
        self._guest     = ''
        self._game_color = None   # màu được server assign
    ...
    def _handle(self, event):
        ...
        if self.btn_start.handle_event(event):
            if self.is_host and self._guest:
                if self._client:
                    self._client.emit('start_game', {'pin': self.pin})
            elif self.is_host:
    ...
    def _draw_panel(self, surface, ox, oy):
        ...
        hl = self.f_player.render(self.display_name if self.is_host else self.host,
                                  True, C_HOST_COLOR)
        surface.blit(hl, (host_bg.x+38, host_bg.centery-hl.get_height()//2))
        rl = self.f_small.render('Chu phong', True, C_TEXT_DIM)
        surface.blit(rl, (host_bg.right-rl.get_width()-10,
                          host_bg.centery-rl.get_height()//2))
        ...
        if self._guest:
            ...
            surface.blit(gr, (guest_bg.right-gr.get_width()-10,
                              guest_bg.centery-gr.get_height()//2))
        else:
