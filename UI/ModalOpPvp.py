...
from LoginAndResgister import (C_PANEL, C_PANEL2, C_BORDER, C_ACCENT, C_ACCENT_HOV,
    C_TEXT, C_TEXT_DIM, C_OVERLAY, C_SUCCESS, C_ERROR, InputField, Button)
from socket_client import SocketClient
from server_config import SERVER_URL

C_CREATE_BG=(50,160,100); C_CREATE_HOV=(70,200,130)
...
class ModalOpPvp:
    ...
    def __init__(self, screen_w, screen_h, username='Guest', display_name=''):
        ...
        self.username=username; self.display_name=display_name or username
        self._result=...; self._open_t=0
        self._msg=''; self._msg_ok=False
    ...
    def _build(self):
        ...
        self.field_pin=InputField(mx+pad,my+72,iw-110,40,label='Ma PIN phong',placeholder='6 chu so...')
        self.btn_join=Button(mx+pad+iw-100,my+72,100,40,text='Vao phong',bg=C_JOIN_BG,bg_hover=C_JOIN_HOV,text_color=(255,255,255),radius=10)
        self.btn_create=Button(mx+pad,my+148,iw,44,text='+ Tao phong moi',bg=C_CREATE_BG,bg_hover=C_CREATE_HOV,text_color=(255,255,255),radius=12)
    ...
    def _cleanup(self):
        if self._client and self._my_pin:
            self._client.emit('leave_room',{'pin':self._my_pin,'username':self.username})
    ...
    def _handle(self, event):
        ...
        if event.type==pygame.MOUSEBUTTONDOWN:
            if self.btn_close.collidepoint(event.pos): self._cleanup(); self._result=None; return
            if not self.panel_rect.collidepoint(event.pos): self._cleanup(); self._result=None; return
            row_h=44; lr=self._list_rect
            for i,room in enumerate(self._rooms[:8]):
                if pygame.Rect(lr.x,lr.y+i*row_h,lr.w,row_h-2).collidepoint(event.pos):
                    self._do_join(room['pin']); return
        self.field_pin.handle_event(event)
        if self.btn_join.handle_event(event): self._do_join(self.field_pin.text.strip())
        if self.btn_create.handle_event(event): self._do_create()
    ...
    def _draw_panel(self, surface, ox, oy):
        ...
        if self._connecting: st=self.f_small.render('⏳ Dang ket noi...',True,C_TEXT_DIM)
        elif self._connected: st=self.f_small.render('● Da ket noi Railway',True,C_SUCCESS)
        else: st=self.f_small.render('● Mat ket noi server',True,C_ERROR)
        ...
        pad=28
        orig=self.field_pin.rect.copy(); self.field_pin.rect=self.field_pin.rect.move(dx,dy)
        self.field_pin.draw(surface,self.f_label,self.f_input); self.field_pin.rect=orig
        orig=self.btn_join.rect.copy(); self.btn_join.rect=self.btn_join.rect.move(dx,dy)
        self.btn_join.draw(surface,self.f_small); self.btn_join.rect=orig
        sep_y=oy+128
        ...
        old_clip=surface.get_clip(); surface.set_clip(clip_r)
        if not self._rooms:
            ...
            for i,room in enumerate(self._rooms[:8]):
                ry=list_y+i*row_h; bg=C_ROOM_ROW_A if i%2==0 else C_ROOM_ROW_B
